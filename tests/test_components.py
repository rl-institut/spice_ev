...
from spice_ev import battery, charging_curve


class TestChargingCurve:
    def test_creation(self):
        # basic charging curve
...
class TestBattery:
    ...
    def test_charging(self):
        ...
        p = b.charge(td, target_power=10)["avg_power"]
        ...
        with pytest.raises(Exception):
            b.charge(td, target_power=10, target_soc=1)

        # test max_power
        b.soc = 0.1
        p = b.charge(td, max_power=10)["avg_power"]
        assert pytest.approx(p) == 10
        b.soc = 0.1
        p = b.charge(td, max_power=100)["avg_power"]
        assert pytest.approx(p) == 42

        # test target_soc
        b.soc = 0.1
        b.charge(td, target_soc=0.2)
        ...
        p1 = b1.charge(td, target_power=10)["avg_power"]
        p2 = 0
        for _ in range(10):
            p2 += b2.charge(td, target_power=1)["avg_power"]
        ...
        # make sure charging piecewise gives same result as charging once for same duration
        b1 = battery.Battery(100, lc, 0, discharging_curve=lc)
        b2 = battery.Battery(100, lc, 0, discharging_curve=lc)
        td = datetime.timedelta(hours=1)
        p1 = b1.discharge(td, target_soc=-float('inf'))["avg_power"]
        p2 = 0
        for _ in range(10):
            p2 += b2.discharge(td, p1/10, target_soc=-float('inf'))["avg_power"]
        assert pytest.approx(b1.soc) == b2.soc, "SoC different: {} vs {}".format(b1.soc, b2.soc)
        assert pytest.approx(p1) == p2, "Used power different: {} vs {}".format(p1, p2)
        ...
        td = datetime.timedelta(hours=1)
        p1 = b1.charge(td*10, target_power=42)["avg_power"]
        p2 = 0
        for _ in range(10):
            p2 += b2.charge(td, target_power=42)["avg_power"]
        p2 /= 10
        ...
        # check that charging and uncharging behave the same given same charging curve
        b1 = battery.Battery(100, lc, 0, efficiency=1)
        b2 = battery.Battery(100, lc, 1, efficiency=1, discharging_curve=lc)
        td = datetime.timedelta(minutes=15)
        t1 = t2 = 0
        while not pytest.approx(b1.soc) == 1:
            b1.charge(td, target_soc=1)["avg_power"]
            t1 += 1
        while not pytest.approx(b2.soc) == 0:
            b2.discharge(td, target_soc=0)["avg_power"]
            t2 += 1
        assert t1 == t2, "Charging(0-100) and uncharging(100-0) processes vary in duration"
    ...
    def test_initial_soc_negative(self):
        ...
        # 1. The amount of energy (dis)charged within an hour is correct
        ...
        for t in target:
            b.soc = initial_soc
            p = b.charge(td_short, 10, target_soc=t[0])["avg_power"]
            assert pytest.approx(p) == t[1]
            assert pytest.approx(b.soc) == t[2]
            p = b.charge(td_long, 50, target_soc=t[0])["avg_power"]
        ...
        for t in target:
            b.soc = initial_soc
            p = b.discharge(td_short, max_power=10, target_soc=t[0])["avg_power"]
            assert pytest.approx(p) == t[1]
            assert pytest.approx(b.soc) == t[2]
            p = b.discharge(td_long, max_power=50, target_soc=t[0])["avg_power"]
    ...
    def test_differential(self):
        ...
        b.discharge(datetime.timedelta(hours=1))
    ...
    def test_discharge(self):
        ...
        td = datetime.timedelta(hours=1)
        # simple uncharging
        pwr = b.discharge(td)['avg_power']
        assert pytest.approx(pwr, 2) == 2.76
        b.soc = 0.5
        # target soc
        pwr = b.discharge(td, target_soc=0.4)['avg_power']
        assert pytest.approx(pwr) == 0.75
        assert b.soc == 0.4
        b.soc = 0.5
        # target power
        pwr = b.discharge(td, target_power=1)['avg_power']
        assert pytest.approx(pwr) == 1
        # target soc and power not allowed
        with pytest.raises(AssertionError):
            b.discharge(td, target_soc=1, target_power=1)

    def test_overcharge(self):
        lc = charging_curve.ChargingCurve([(0, 10), (1, 10)])
        capacity = 10
        efficiency = 1
        initial_soc = 0.5
        b = battery.Battery(capacity, lc, initial_soc, efficiency, lc)
        ...
        # draw above 100% soc
        b.charge(td, max_power=10, target_soc=1.5)
        assert b.soc == 1

        # discharge below 0% soc
        b.soc = 0.5
        b.discharge(td, target_soc=-1)
        assert b.soc == 0

        # target soc below initial soc
        b.soc = 0.5
        b.charge(td, max_power=10, target_soc=0)
        assert b.soc == 0.5

        # supply 10 kW over 1 h from 5 kWh stored energy: don't go below 0
        b.soc = 0.5
        b.discharge(td, target_power=10)
        assert pytest.approx(b.soc) == 0

        # supply 10 kW over 1 h from negative stored energy: don't discharge
        b.soc = -0.5
        b.discharge(td, target_power=10)
        assert b.soc == -0.5
        b.soc = -0.5
        b.discharge(td, target_soc=0)
        assert b.soc == -0.5
    ...
    def test_changing_charging_curve(self):
        ...
        lc = charging_curve.ChargingCurve([[0, 50], [0.7, 30], [0.9, 50], [1, 5]])
        capacity = 315
        efficiency = 0.95
        ...
        p = b.charge(td, target_power=target_power)['avg_power']
        ...
        lc = charging_curve.ChargingCurve([(0, 0.0), (0.8, 0.0), (1, 0.0)])
        b = battery.Battery(350, lc, 0.8)
        td = datetime.timedelta(minutes=15)
        p = b.charge(td)["avg_power"]
        assert p == 0

    def test_infinite_capacity(self):
        lc = charging_curve.ChargingCurve([(0, 100), (1, 100)])
        b = battery.Battery(capacity=2**64, charging_curve=lc, soc=0, efficiency=1)
        target = 50
        td = datetime.timedelta(minutes=60)
        p = b.charge(td, target_power=target)["avg_power"]
        assert pytest.approx(p) == target

        # does it work for a range of capacities?
        for i in range(0, 65):
            capacity = 2**i
            b = battery.Battery(capacity=capacity, charging_curve=lc, soc=0, efficiency=1)
            p = b.charge(td, target_power=target)["avg_power"]
