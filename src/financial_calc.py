...
class Financial:
    ...
    def setup_capex(self, capex_dict):
    ...
    def battery_cost(self, capacity, c_rate=1):
        ...
        self.battery_cost_dict[0] = {
            "value": [c_rate, 2109.62368 / 1e3, -147.52325 / 1e6, 6.97016 / 1e9, -0.13996 / 1e12,
                      0.00102 / 1e15]}
        ...
        if capacity < 50:
            cost = get_cost(capacity * self.battery_scale,
                            {**self.battery_cost_dict[0], **self.battery_cost_type})
        elif capacity < 1000:
            cost = get_cost(capacity * self.battery_scale,
                            {**self.battery_cost_dict[50], **self.battery_cost_type})
        elif capacity >= 1000:
