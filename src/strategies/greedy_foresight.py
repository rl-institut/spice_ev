    ...
    def step(self, event_list=[]):
        ...
        vehicles = sorted(
            ...
            key=lambda v: (v.battery.capacity * v.battery.soc /
                           self.world_state.charging_stations[
                            v.connected_charging_station].max_power) /
                          ((v.estimated_time_of_departure - self.current_time) / self.interval))
        """
        # order by time of departure
        vehicles = sorted(
            [v for v in self.world_state.vehicles.values()
                if v.connected_charging_station is not None],
            key=lambda v: v.estimated_time_of_departure)
        ...
        for vehicle in vehicles:
            ...
            if cost_for_one <= self.PRICE_THRESHOLD:
                ...
                departure_index = int(
                    (vehicle.estimated_time_of_departure - self.current_time) / self.interval)
                for (cost, ts_idx) in sorted_ts:
                    ...
                    if cost <= self.PRICE_THRESHOLD:
                        ...
                        if sim_vehicle.battery.soc >= vehicle.desired_soc:
                            # max power charges too much
                            # -> find minimum viable power to reach desired SoC
                            while max_power - min_power > self.EPS:
                                ...
                                avg_power = sim_vehicle.battery.load(
                                    self.interval, cur_power)['avg_power']
                                if sim_vehicle.battery.soc < vehicle.desired_soc:
                                    # not enough power
                                    min_power = cur_power
                                else:
