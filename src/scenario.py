...
class Scenario:
    ...
    def run(self, strategy_name, options):
        ...
        options["DISCHARGE_LIMIT"] = options.get(
        ...
        for step_i in range(self.n_intervals):
            ...
            for gcID, gc in strat.world_state.grid_connectors.items():
                ...
                for vidx, vid in enumerate(sorted(strat.world_state.vehicles.keys())):
                    vehicle = strat.world_state.vehicles[vid]
                    if vehicle.connected_charging_station and (
        ...
        for gcID in gc_ids:
            print(
                "Energy drawn from {}: {:.0f} kWh, Costs: {:.2f} €".format(
                    gcID, sum(totalLoad[gcID]) / stepsPerHour, sum(costs[gcID])
                )
            )
        ...
        if options.get("save_timeseries", False):
            ...
            for gcID in gc_ids:
                ...
                (
                    ...
                    power_feed_in_list,
                    charging_signal_list,
                ) = report.save_gc_timeseries(self, strategy_name, gcID, output_path)

        if options.get("save_soc", False) or options.get("attach_vehicle_soc", False):
            self.vehicle_socs = {}
            report.save_soc_timeseries(
                scenario=self, output_path=options.get("save_soc", None)
            )
        ...
        if options.get("testing", False):
            self.testing = {
                ...
                "sum_feed_in_per_h": {
                    gcID: (sum(feedInPower[gcID]) / stepsPerHour) for gcID in gc_ids
                },
